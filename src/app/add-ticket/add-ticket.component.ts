...
export class AddTicketComponent implements OnInit {
 ...
 mydate=new Date()
  ...
  AddDemande() {
    ...
    console.log(data);
    // Extraction du l'identifiant de projet a partir du  demande 
    this.service.getDemandeById(parseInt(data.tache_patent)).subscribe(demande=>{
      this.currentProjet=demande.projet
    ...
    let demande = new Demande(
      undefined,data.tracker,data.sujet,this.texteSansBalises,data.statut,data.priorite,data.assigne,data.version,data.tache_patent,this.mydate.toISOString().slice(0,10),data.echeance,
      data.temps_estime,data.facture,this.imgURL,JSON.parse(JSON.stringify(this.currentProjet)));
       console.log(demande);
    if(data.tacker==0||data.sujet==0)
    {this.toast.info({
      detail:'error msg !!',
